...
func (s *Server) Handler() http.Handler {
	...
	mux.HandleFunc("GET /api/v1/envs/{env}/defaults/sidecar-definitions/{sidecar_name}/resources", s.handleDefaultsSidecarDefinitionResources)
	mux.HandleFunc("PATCH /api/v1/envs/{env}/defaults/sidecar-definitions/{sidecar_name}/resources", s.handleDefaultsSidecarDefinitionResourcesUpdate)
	mux.HandleFunc("GET /api/v1/envs/{env}/defaults/sidecar-definitions/{sidecar_name}/envs", s.handleDefaultsSidecarDefinitionEnvs)
	mux.HandleFunc("PATCH /api/v1/envs/{env}/defaults/sidecar-definitions/{sidecar_name}/envs", s.handleDefaultsSidecarDefinitionEnvsUpdate)
	mux.HandleFunc("GET /api/v1/envs/{env}/defaults/sidecar-definitions/{sidecar_name}/references", s.handleDefaultsSidecarDefinitionReferences)
	mux.HandleFunc("PATCH /api/v1/envs/{env}/defaults/sidecar-definitions/{sidecar_name}/references", s.handleDefaultsSidecarDefinitionReferencesUpdate)
...
func (s *Server) handleDefaultsSidecarDefinition(w http.ResponseWriter, r *http.Request) {
	if s.repo == nil {
		writeError(w, http.StatusServiceUnavailable, "repository root is not configured")
		return
	}
	definition, err := s.repo.DefaultsSidecarDefinition(r.PathValue("env"), r.PathValue("sidecar_name"))
	if err != nil {
		writeError(w, statusForError(err), err.Error())
		return
	}
...
func (s *Server) handleDefaultsSidecarDefinitionUpdate(w http.ResponseWriter, r *http.Request) {
	...
	var payload struct {
		ExpectedHash string                                     `json:"expected_hash"`
		Definition   repository.DefaultsSidecarDefinitionUpdate `json:"definition"`
	}
	if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	definition, err := s.repo.UpdateDefaultsSidecarDefinition(r.PathValue("env"), r.PathValue("sidecar_name"), payload.Definition, payload.ExpectedHash)
	if err != nil {
		writeError(w, statusForError(err), err.Error())
		return
	}
...
func (s *Server) handleDefaultsSidecarDefinitionResources(w http.ResponseWriter, r *http.Request) {
	if s.repo == nil {
		writeError(w, http.StatusServiceUnavailable, "repository root is not configured")
		return
	}
	resources, err := s.repo.DefaultsSidecarDefinitionResources(r.PathValue("env"), r.PathValue("sidecar_name"))
	if err != nil {
		writeError(w, statusForError(err), err.Error())
		return
	}
...
func (s *Server) handleDefaultsSidecarDefinitionResourcesUpdate(w http.ResponseWriter, r *http.Request) {
	if s.repo == nil {
		writeError(w, http.StatusServiceUnavailable, "repository root is not configured")
		return
	}
	...
	var payload struct {
		ExpectedHash string                                              `json:"expected_hash"`
		Resources    repository.DefaultsSidecarDefinitionResourcesUpdate `json:"resources"`
	}
	if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	resources, err := s.repo.UpdateDefaultsSidecarDefinitionResources(r.PathValue("env"), r.PathValue("sidecar_name"), payload.Resources, payload.ExpectedHash)
	if err != nil {
		writeError(w, statusForError(err), err.Error())
		return
	}
...
func (s *Server) handleDefaultsSidecarDefinitionEnvs(w http.ResponseWriter, r *http.Request) {
	if s.repo == nil {
		writeError(w, http.StatusServiceUnavailable, "repository root is not configured")
		return
	}
	envs, err := s.repo.DefaultsSidecarDefinitionEnvs(r.PathValue("env"), r.PathValue("sidecar_name"))
	if err != nil {
		writeError(w, statusForError(err), err.Error())
		return
	}
...
func (s *Server) handleDefaultsSidecarDefinitionEnvsUpdate(w http.ResponseWriter, r *http.Request) {
	if s.repo == nil {
		writeError(w, http.StatusServiceUnavailable, "repository root is not configured")
		return
	}
	...
	var payload struct {
		ExpectedHash string                                         `json:"expected_hash"`
		Envs         repository.DefaultsSidecarDefinitionEnvsUpdate `json:"envs"`
	}
	if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	envs, err := s.repo.UpdateDefaultsSidecarDefinitionEnvs(r.PathValue("env"), r.PathValue("sidecar_name"), payload.Envs, payload.ExpectedHash)
	if err != nil {
		writeError(w, statusForError(err), err.Error())
		return
	}
...
func (s *Server) handleDefaultsSidecarDefinitionReferences(w http.ResponseWriter, r *http.Request) {
	if s.repo == nil {
		writeError(w, http.StatusServiceUnavailable, "repository root is not configured")
		return
	}
	references, err := s.repo.DefaultsSidecarDefinitionReferences(r.PathValue("env"), r.PathValue("sidecar_name"))
	if err != nil {
		writeError(w, statusForError(err), err.Error())
		return
	}
...
func (s *Server) handleDefaultsSidecarDefinitionReferencesUpdate(w http.ResponseWriter, r *http.Request) {
	if s.repo == nil {
		writeError(w, http.StatusServiceUnavailable, "repository root is not configured")
		return
	}
	...
	var payload struct {
		ExpectedHash string                                               `json:"expected_hash"`
		References   repository.DefaultsSidecarDefinitionReferencesUpdate `json:"references"`
	}
	if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	references, err := s.repo.UpdateDefaultsSidecarDefinitionReferences(r.PathValue("env"), r.PathValue("sidecar_name"), payload.References, payload.ExpectedHash)
	if err != nil {
		writeError(w, statusForError(err), err.Error())
		return
	}
